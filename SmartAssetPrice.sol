...
contract SmartAssetPrice {
     ...
     */
    function calculateAssetPrice(uint assetId)  onlySmartAsset returns (bool result){
        SmartAssetInterface asset = SmartAssetInterface(smartAssetAddr);
        var(id, b1, b2, b3, u1, u2, bool1, state) =  asset.getAssetById(assetId);
        uint price = _calculateAssetPrice(u1, u2, bool1);

        SmartAssetPriceData memory smartAssetPriceData = SmartAssetPriceData(price, sha256(id, b1, b2, b3, u1, u2, bool1, state));
    ...
    function checkSmartAssetModification(uint assetId) constant returns (bool modified) {
        ...
        var(id, b1, b2, b3, u1, u2, bool1, state) =  asset.getAssetById(assetId);
        //check scenario when there is no id in map
        return sha256(id, b1, b2, b3, u1, u2, bool1, state) == smartAssetPriceById[assetId].hash;
    }
